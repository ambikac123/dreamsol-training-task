...
public class ExcelHelper<T>
{
    ...
    public <T> Resource getAutoGeneratedExcelFile()
    {
        ...
        for(int x=1; x<=10;x++)
        {
            ...
            for(i=1;i<=10;i++)
            {
                ...
                if(i<=7)
                    code =code + chars[random.nextInt(26)];
            }
            d.setDepartmentName(name);
            d.setDepartmentCode(code);
            d.setStatus(random.nextBoolean());
            departmentList.add(d);
        }

        ByteArrayInputStream byteArrayInputStream = convertListToExcel(Department.class,departmentList,headersMap,sheetName);
        return new InputStreamResource(byteArrayInputStream);
    }
}
